...
import br.com.embracon.teamposition.domain.service.CollaboratorService;
...
public class TeamSellerBean extends DefaultBean {
	...
	public void searchAdministrative() {
		...
		try {
			...
			Collaborator collaborator = collaboratorService.findCollaborator(this.administrativeSearch);
			if(collaborator != null && !this.administratives.contains(collaborator)) {
				this.administratives.add(collaborator);
			}
	...
	public void findSeller() {
		...
		try {
			...
			this.seller = collaboratorService.findCollaborator(
					this.sellerSearch, SoldCodeEnum.SELLER, Boolean.FALSE);

			this.service.existCollaboratorInOtherBranch(
					this.seller, this.branchIdSelected);
	...
	public void findSupervisor() {
		...
		try {
			...
			this.supervisor = collaboratorService.findCollaborator(
					this.supervisorSearch, SoldCodeEnum.SUPERVISOR, Boolean.FALSE);

			this.service.existCollaboratorInOtherBranch(
					this.supervisor, this.branchIdSelected);

			Integer amount = this.collaboratorService.countTeam(this.supervisor);
			Configuration teamConfiguration = configurationService.findByClazzAndType(
					TeamSeller.class, ConfigurationEnum.SUPERVISOR_TEAM);
			if(amount > teamConfiguration.getIntegerValue()) {
				warningMessage("validation.amount.collaborator.team", 
						this.supervisor.getName(), amount);
			}
		} catch (ValidationException e) {
			warningMessage(e.getResult().getReasons());
			this.seller = new Collaborator();
